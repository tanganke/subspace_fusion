...
class Program(_Program):
    def __init__(self, cfg: DictConfig):
        ...
        self.results_path = self.results_dir / "layer_wise_concrete_adamerging.csv"
        self.ckpt_dir = self.results_dir / "layer_wise_concrete_adamerging"
        self.ckpt_path = self.ckpt_dir / "ckpt.pt"
        self.individual_results_path = self.results_dir / "layer_wise_concrete_adamerging_individuals.csv"
        self.tta_ckpt_dir = self.results_dir / "layer_wise_concrete_adamerging_tta"
        self.tta_ckpt_path = self.tta_ckpt_dir / "ckpt.pt"
        self.tta_results_path = self.results_dir / "layer_wise_concrete_adamerging_tta.csv"
    ...
    def meta_train(self):
        ...
        base_optimizer = torch.optim.Adam(self.model.parameters(), lr=0.1, betas=(0.9, 0.999), weight_decay=0.0)
        ...
        for epoch_idx in tqdm(range(epochs := 2000)):
            # update task-wise weights
            self.model.layer_wise_weight.data = deepcopy(self.init_layer_wise_weights).to(self.model.layer_wise_weight.device)
            self.model.train()
