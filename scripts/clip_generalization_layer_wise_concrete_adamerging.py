...
from src.layer_wise_fusion import check_parameterNamesMatch
...
class Program:
    def __init__(self, cfg: DictConfig):
        ...
        self.results_path = result_dir / "layer_wise_concrete_adamerging.csv"
        self.ckpt_dir = result_dir / "layer_wise_concrete_adamerging"
        self.ckpt_path = self.ckpt_dir / "ckpt.pt"
        self.individual_results_path = result_dir / "layer_wise_concrete_adamerging_individuals.csv"
        self.tta_ckpt_dir = result_dir / "layer_wise_concrete_adamerging_tta"
        self.tta_ckpt_path = self.tta_ckpt_dir / "ckpt.pt"
        self.tta_results_path = result_dir / "layer_wise_concrete_adamerging_tta.csv"
    ...
    def initialize_merged_model(self):
        ...
        self.init_layer_wise_weight = get_layer_wise_weights(
        ...
        model = LayerWiseMergedModel(
            ...
            layer_wise_weight=deepcopy(self.init_layer_wise_weight),
    ...
    def meta_train(self):
        ...
        for epoch_idx in tqdm(range(epochs := 2000)):
            self.model.layer_wise_weight.data = deepcopy(self.init_layer_wise_weight).to(self.model.layer_wise_weight.device)
            self.model.train()
            losses = 0
            # * only seen datasets are used for AdaMerging
    ...
    def eval_individuals(self):
        ...
        results = {"dataset": [], "acc": []}
        Total_ACC = 0
        for dataset_idx, dataset_name in enumerate(tqdm(cfg.seen_datasets, desc="evaluating individual models")):
    ...
    def adamerging(self):
        ...
        for epoch_idx in tqdm(range(epochs := 1000)):
            self.model.train()
            losses = 0
            for dataset_name in self.cfg.seen_datasets:  # * only seen datasets are used for AdaMerging
                batch = next(self.shuffled_test_loader_iters[dataset_name])
                batch = maybe_dictionarize(batch)
                x = batch["images"].to(cfg.device)
