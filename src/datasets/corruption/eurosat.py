...
class EuroSATBase:
    def __init__(
        ...
        if corruption is not None:
            print(">>>> Eurosat corruption style:" + corruption)
            testdir = os.path.join(
                location, "EuroSAT_splits", "test_Corruption/severity_2/" + corruption
            )
...
class EuroSAT(EuroSATBase):
    def __init__(
        ...
        logger.info(f"Loading EuroSAT dataset with corruption {corruption}.")
        super().__init__(
            preprocess,
            "test_Corruption",
            location,
