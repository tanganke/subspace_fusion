...
class RESISC45Dataset(VisionClassificationDataset):
    ...
    * https://doi.org/10.1109/jproc.2017.2675998
    ...
    directory = "resisc45/NWPU-RESISC45_Corruption/severity_2/"
    ...
    def __init__(
        ...
        # super().__init__(root=os.path.join(root, self.directory), transforms=transforms, is_valid_file=is_in_split,)
        if corruption is not None:
            print(">>>> Resisc45 corruption style:" + corruption)
            super().__init__(
                root=os.path.join(root, self.directory + corruption),
...
class RESISC45:
    def __init__(
        ...
        logger.info(f"Loading RESISC45 dataset with corruption {corruption}.")

        self.train_dataset = RESISC45Dataset(
            root=location, split="train", transforms=preprocess, corruption=corruption
        )
        ...
        self.test_dataset = RESISC45Dataset(
            root=location, split="test", transforms=preprocess, corruption=corruption
        )
